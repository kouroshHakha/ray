...
class EpisodeV3:
    ...
    def __init__(
        ...
        policy_mapping_fn: Callable[[AgentID, "EpisodeV3", "RolloutWorker"], PolicyID],
        *,
        ...
        Args:
            ...
            marl_module: MultiAgentRLModule instance.
            policy_map: The PolicyMap object (mapping PolicyIDs to Policy
                objects) to use for determining, which policy is used for
                which agent. This is only used here for postprocess-trajectory().
        ...
        self.algo_config: AlgorithmConfig = self.worker.config

        self.marl_module = marl_module
        # this is still needed for the postprocess_trajectory component
        self.policy_map = policy_map
        self.policy_mapping_fn: Callable[
            [AgentID, "EpisodeV3", "RolloutWorker"], PolicyID
        ] = policy_mapping_fn
    ...
    def add_init_obs(
        ...
        Args:
            ...
            t: timestamp.
        """
        rl_module = self.marl_module[self.policy_for(agent_id)]

        # Add initial obs to Trajectory.
        assert agent_id not in self._agent_collectors
        init_state = rl_module.get_initial_state()
        self._agent_collectors[agent_id] = AgentCollector(
            rl_module.view_requirements,
            max_seq_len=self.algo_config.model["max_seq_len"],
            disable_action_flattening=self.algo_config.get(
                "_disable_action_flattening", False
            ),
            is_policy_recurrent=len(init_state) > 0,
            intial_states=init_state,
        )
    ...
    def postprocess_episode(
        self,
        batch_builder: MultiAgentRLModuleTrajectoryCollector,
        is_done: bool = False,
        ...
        Args:
            batch_builder: MultiAgentRLModuleTrajectoryCollector for saving the collected per-agent
                sample batches.
        ...
        for agent_id, (pid, policy, pre_batch) in pre_batches.items():
            ...
            if pid not in batch_builder.policy_collectors:
                batch_builder.policy_collectors[pid] = RLModuleTrajectoryCollector()
