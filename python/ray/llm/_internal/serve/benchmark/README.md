...
- This is a **simple-mode-only** tool. There is no expert mode for specifying
  raw `u`/`s` values directly, no `generation-osl` parameter, and no built-in
  sweep orchestration.
- External scripts can loop over the CLI for parameter sweeps.
- The interactive server communicates via a UNIX domain socket
  (`/tmp/interactive_rate_bench.sock` by default, configurable via
  `RAY_BENCH_CONTROL_SOCKET` environment variable).
